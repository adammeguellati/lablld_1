...
board format used by `lablld-board.json`.
...
change to make it live. Verify with
`node docs/board/validate-board.mjs docs/board/BOARD-TEMPLATE.json`.
...
4. Replace the eight `G1..G8` conditions with the real launch conditions. If the
   count is not eight, change `LAUNCH_GATE_DENOMINATOR` in `validate-board.mjs`
