...
//
// Governs the LABLLD board:
//   docs/board/lablld-board.json  "LABLLD Build Board"
//
...
//         (defaults to ./lablld-board.json next to this script)
//
// Zero dependencies. Node ESM. Never writes; read-only.
//
// PROVENANCE. This script was ported from the OsteoJP board system (see
// docs/board/PORT-NOTES.md for every patch applied). Comments below still cite
// OsteoJP documents - docs/loops/README.md, WAVE-13.md, PORTAL-REHYDRATE.md,
// docs/design/BACKLOG.md, EXTERNAL-AGENDA.md, external-agenda-ledger.test.mjs -
// which do NOT exist in this repo. They are kept as the record of WHY each rule
// exists; the rules themselves are project-neutral and enforce fine here. Do not
// treat those paths as live references.
...
const boardPath = resolve(process.argv[2] ?? `${HERE}/lablld-board.json`);
...
const KIND_LANES = ["in_flight", "adam_batch", "incidents", "loose_ends"];
const PRIORITY = ["high", "medium", "low"];
// The boards this script governs. The name is a membership check rather than a
// second copy of this file, so a second LABLLD board would be added here.
const BOARD_NAMES = ["LABLLD Build Board"];
// The BLOCKED ON PEOPLE lane is split by person. The set is read from that
// lane's own `columns` (below, after the board loads); this is only the fallback
// for a board that omits them. On LABLLD the two people who can owe an answer
// are Ivan (operator) and Adam (owner).
const DEFAULT_PEOPLE = ["ivan", "adam"];
let PEOPLE = DEFAULT_PEOPLE;
// LABLLD gate vocabulary. `green_self_merge` denotes an executor-terminal
// self-merge; `ivan_merge` is Ivan clearing the merge; `adam_authorizes` is the
// owner authorising the spend, the account change or the product decision;
// `stakeholder` is an outside party (Shopify app review, counsel, a processor).
const GATE = [
  "green_self_merge",
  "ivan_merge",
  "adam_authorizes",
...
// LABLLD's launch gate is EIGHT conditions, not nine. BOARD-TEMPLATE.README.md
// step 4: change this and DEFAULT_DENOMINATOR in board-config.mjs together.
const LAUNCH_GATE_DENOMINATOR = 8;
