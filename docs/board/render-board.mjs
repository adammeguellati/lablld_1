...
//
// Which board it renders is decided by the JSON's own `board` field, which
// docs/board/board-config.mjs maps to that board's display configuration. An
// unknown board name is a hard error, never a silent fallback to another
// board's identity.
//
...
//         defaults: ./lablld-board.json  ->  ./lablld-board.rendered.html
...
// The default out path follows the board's own configured render path.
 ...
 *
 * THE CARDS STAY IN THE JSON — the ledger is the point, and a deleted card takes
 * its history with it — but a card on a rendered engineering board reads as work
 * engineering owes, and duplicate tracking is what the flag removes.
 *
 * DORMANT ON THIS BOARD: no LABLLD card carries `external_agenda: true` yet, so
 * this filter removes nothing today. The mechanism is kept because the field is
 * still validated. If it is ever used, create docs/board/EXTERNAL-AGENDA.md —
...
const lg = visible.launch_gate ?? { denominator: 8, conditions: [] };
const passed = (lg.conditions ?? []).filter((g) => g.state === "pass").length;
const denom = lg.denominator ?? 8;
