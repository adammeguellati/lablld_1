/* board-app.js - client runtime for the board portal artifacts.
   ...
   `home_lane` is the card's KIND (work item / incident / Adam decision / loose
...
(function () {
   ...
   * "ivan", "adam", "LABLLD" or "lablld-board.json" are allowed to appear. It is
   * the fallback for a page rendered without a #board-config island (an older
   * renderer, or a hand-saved copy). Kept in lock-step with
   * docs/board/board-config.mjs, which is where the real configuration lives and
   * where a new board is added.
   */
  var FALLBACK_CONFIG = {
    boardName: "LABLLD Build Board",
    owner: "ivan",
    people: ["ivan", "adam"],
    whoOrder: [null, "ivan", "adam", "infra"],
    whoLabels: { ivan: "Ivan", adam: "Adam", infra: "Infra" },
    laneLabels: {
      blocked_on_people: "Blocked on people", in_flight: "In flight",
      adam_batch: "Adam batch", incidents: "Incidents",
      loose_ends: "Loose ends", shipped: "Shipped",
    },
    laneHints: {
      blocked_on_people: "someone owes an answer", in_flight: "the active work queue",
      adam_batch: "waiting on an Adam decision", incidents: "live problems",
      loose_ends: "tracked, not queued", shipped: "done, with proof",
    },
    kindLabels: {
      in_flight: "Work item", adam_batch: "Adam decision",
      incidents: "Incident", loose_ends: "Loose end",
    },
    denominator: 8,
    denominatorWord: "Eight",
  ...
  var PORTAL_GEN = "p2";
  var STORAGE_KEY = "lablld-board:" + (SEED.board || "board") + ":v" + (SEED.schema_version || 1) + ":" + PORTAL_GEN;
  var UI_KEY = STORAGE_KEY + ":ui";

  var KIND_LANES = ["in_flight", "adam_batch", "incidents", "loose_ends"];
  var ALL_LANES = ["blocked_on_people", "in_flight", "adam_batch", "incidents", "loose_ends", "shipped"];
  // Display vocabulary is per board; the lane IDs above are structure. Upstream
  // the inbox lane's id was `rodica_batch` on both OsteoJP boards; here it is
  // `adam_batch` (PORT-NOTES.md, patch P3).
  ...
  var GATE_ORDER = ["green_self_merge", "ivan_merge", "adam_authorizes", "stakeholder"];
  var GATE_BADGE = {
    green_self_merge: { label: "Self-merge", cls: "selfmerge" },
    ivan_merge: { label: "Ivan merge", cls: "" },
    adam_authorizes: { label: "ADAM", cls: "autorizo" },
    stakeholder: { label: "Stakeholder", cls: "stakeholder" },
  };
  // The people set is board-relative: ivan/adam here.
  // WHO_ORDER is `blocked_on`'s full domain.
  ...
  function openCardModal(card, defaults) {
    ...
    var c = card ? clone(card) : {
      ...
      owner_terminal: CFG.ownerTerminalDefault, gate: "ivan_merge", evidence: null, blocked_on: defaults.who || null,
